...
function getWeekLabel(): string {
  ...
  end.setDate(end.getDate() - 1); // hasta el domingo anterior
  const start = new Date(end);
  start.setDate(start.getDate() - 6); // desde el lunes
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // Seguridad: solo Vercel Cron o llamada manual con token correcto
  ...
  try {
    const { sales, leads } = await getAndClearWeeklyData();
    const weekLabel = getWeekLabel();

    await sendWeeklyReport(sales, leads, weekLabel);

    // WhatsApp a Mauro como confirmación
    ...
    console.log(`Reporte semanal enviado — ${sales.length} ventas, ${leads.length} cierres`);
    return res.status(200).json({ ok: true, sales: sales.length, leads: leads.length });
  } catch (err) {
